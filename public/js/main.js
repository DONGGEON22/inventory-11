// 메인 애플리케이션 파일
import { showLoading, hideLoading, showToast, showModal, formatCurrency, generateId, renderPagination, showLoadingWithDelay, showProgressLoading, lazyLoad, createSkeletonLoader, createInfiniteScroll } from './ui.js';
...
export async function saveCompanyState() {
    const businessNumber = getCurrentCompanyBusinessNumber();
    if (businessNumber) {
        await saveCompanyData(businessNumber);
    }
}
...
export function loadLogin() {
    ...
    if (businessNumberInput) {
        setupBusinessNumberInput(businessNumberInput, '로그인');
    }
            ...
            if (window.firestore && typeof window.firestore === 'object') {
                ...
                if (approvedUser) {
                    ...
                    if (companyInfo) {
                        ...
                        localStorage.setItem('loginCompanyName', companyInfo.companyName || '');
                        localStorage.setItem('loginCompanyId', companyInfo.id);
...
export async function loadDashboard() {
    ...
    try {
        ...
        if (isAdmin()) {
            // Firestore에서 최신 승인된 사용자 데이터를 가져오기
            let approvedUsers = [];
            try {
...
export function loadPageContent(page) {
    ...
    // 지연로딩을 위한 로딩 컨트롤러
    const loadingController = showLoadingWithDelay('페이지를 불러오는 중...', 300);
    ...
    try {
        switch(page) {
            case 'dashboard':
                ...
                loadDashboard();
                setTimeout(() => loadingController.hide(), 300);
                break;
            case 'items':
                ...
                try {
                    ...
                    if (itemsResult && typeof itemsResult.finally === 'function') {
                        itemsResult.finally(() => loadingController.hide());
                    } else {
                        setTimeout(() => loadingController.hide(), 300);
                    }
                } catch (error) {
            ...
            case 'partners':
                ...
                try {
                    ...
                    if (partnersResult && typeof partnersResult.finally === 'function') {
                        partnersResult.finally(() => loadingController.hide());
                    } else {
                        setTimeout(() => loadingController.hide(), 300);
                    }
                } catch (error) {
            ...
            case 'purchase':
                ...
                try {
                    ...
                    if (purchaseResult && typeof purchaseResult.finally === 'function') {
                        purchaseResult.finally(() => loadingController.hide());
                    } else {
                        setTimeout(() => loadingController.hide(), 300);
                    }
                } catch (error) {
            ...
            case 'sales':
                ...
                try {
                    ...
                    if (salesResult && typeof salesResult.finally === 'function') {
                        salesResult.finally(() => loadingController.hide());
                    } else {
                        setTimeout(() => loadingController.hide(), 300);
                    }
                } catch (error) {
            ...
            case 'monthly':
                ...
                try {
                    ...
                    if (monthlyResult && typeof monthlyResult.finally === 'function') {
                        monthlyResult.finally(() => loadingController.hide());
                    } else {
                        setTimeout(() => loadingController.hide(), 300);
                    }
                } catch (error) {
        ...
        if (mainContent) {
            mainContent.innerHTML = `
                <div class="alert alert-danger text-center mt-5">
                    <h4><i class='bx bx-error-circle me-2'></i>페이지 로드 오류</h4>
                    <p class="mb-3">페이지를 불러오는 중 오류가 발생했습니다.</p>
                    <button class="btn btn-primary" onclick="location.reload()">
                        <i class='bx bx-refresh me-2'></i>페이지 새로고침
                    </button>
                </div>
                ...
                <div class="summary-cards-grid">
                    ...
                    <div class="summary-card">
                        <div class="summary-card-content">
                            <div class="summary-card-header">
                                <div class="summary-card-title">
                                    <i class='bx bx-refresh summary-card-icon'></i>
                                    <span>재고 회전율</span>
                                </div>
                            </div>
                            <div class="summary-card-value-container">
                                <div class="summary-card-value" id="inventoryTurnoverRate">
                                    <span class="loading-spinner"></span>
                                </div>
                                <div class="summary-card-unit">% (12개월 기준)</div>
                            </div>
                        </div>
                    </div>
                </div>
...
/**
 * 재고 회전율 계산 (최근 12개월 기준)
 */
function calculateInventoryTurnoverRate() {
    if (!state.sales || !Array.isArray(state.sales) || !state.items || !Array.isArray(state.items)) {
        return 0;
    }
    ...
    // 최근 12개월간의 총 매출량 계산
    const totalSalesQuantity = state.sales.reduce((total, sale) => {
        if (!sale || !sale.date) return total;
    ...
    // 평균 재고량 계산 (현재 재고량을 대략적인 평균으로 사용)
    const averageInventory = state.items.reduce((total, item) => {
        if (!item) return total;
        ...
        return total + Math.max(0, totalPurchased - totalSold);
    }, 0);
    
    if (averageInventory === 0) return 0;
    return ((totalSalesQuantity / averageInventory) * 100);
}
...
function updateSummaryCards(year, month) {
    ...
    if (turnoverElement) {
        // 숫자인지 확인하고 안전하게 처리
        const rate = typeof turnoverRate === 'number' ? turnoverRate.toFixed(1) : '0.0';
        turnoverElement.innerHTML = rate + '%';
    }
...
export function showSignupModal() {
    ...
    if (businessNumberInput) {
        setupBusinessNumberInput(businessNumberInput, '회원가입');
    }
}
